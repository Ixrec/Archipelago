...
Sometimes the game can be modded to skip these locations or make them less tedious. But if this is about a fundamental aspect of the game like, then the general answer is "soft logic" (and subtypes like "combat logic", "money logic", etc). For example: you can logically require that a player have several useful items before fighting the final boss, even if a skilled player technically needs no items to beat it. Randomizer logic should describe what's *fun* rather than what's technically possible.
...
- Buying shop items might logically require access to an easy way of farming money, or logically require access to enough parts of the game that checking other locations should naturally generate enough money without grinding.
